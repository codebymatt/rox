...
class GenerateAST
  ...
  def define_ast(output_directory, base_class_name, types)
    path = "./#{output_directory}/#{base_class_name}.rb"
    @output_file = File.open(path, 'w')

    @output_file.puts('# frozen_string_literal: true')
    @output_file.puts('')

    @output_file.puts("class #{base_class_name.capitalize}")
    ...
    types.each do |type|
      ...
      define_type(base_class_name.capitalize, sub_class_name, fields)
    end

    @output_file.close
  end

  def define_type(base_class_name, sub_class_name, fields)
    write_class_opening(base_class_name, sub_class_name, fields)

    fields.split(' ')
          .map { |field| field.gsub(',', '') }
          .each { |field| write_type(field) }

    write_visitor_pattern(base_class_name, sub_class_name)
    write_class_ending
  end

  def write_class_opening(base_class_name, sub_class_name, fields)
  ...
  def write_visitor_pattern(base_class_name, sub_class_name)
    visitor_target = "#{sub_class_name.capitalize}#{base_class_name.capitalize}"
    @output_file.puts('')
